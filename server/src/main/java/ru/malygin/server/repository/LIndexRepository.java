...
public interface LIndexRepository extends CrudRepository<LIndex, Long> {
    ...
    @Query(
            value = "select pageId, lemmaRank from (" +
                    "select page_id as pageId, count(page_id) as pageCount, sum(_rank) as lemmaRank from _lindex where lemma_id in (:lemmasId) group by page_id)" +
                    " as tmp where pageCount = (:lemmasCount) order by lemmaRank desc",
            nativeQuery = true)
